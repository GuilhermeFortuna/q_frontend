...
import { useBacktestHistory, useBacktestRun, useDeleteBacktest } from '@/api/queries/backtests'
  ...
  return (
    ...
    >
      ...
      <button
        ...
        aria-label={`Delete run ${run.symbol} ${run.strategy}`}
      >
...
export function BacktestHistoryPanel({
  ...
  const pendingDeleteRun = runs.find((run) => run.run_id === pendingDeleteRunId)

  const handleConfirmDelete = () => {
    if (!pendingDeleteRunId) return
    deleteBacktest.mutate(pendingDeleteRunId, {
      onSuccess: () => {
  ...
  return (
    <>
          ...
          <div className="min-h-0 flex-1 space-y-2 overflow-y-auto p-3">
            ...
            {runs.map((run) => (
              <RunListItem
                ...
                onDelete={() => setPendingDeleteRunId(run.run_id)}
                deleting={deleteBacktest.isPending && pendingDeleteRunId === run.run_id}
              />
            ))}
          </div>
        </div>
      ...
      <ConfirmDialog
        ...
        description={
          pendingDeleteRun
            ? `Remove "${pendingDeleteRun.symbol} · ${pendingDeleteRun.strategy}" from history? This cannot be undone.`
            : 'Remove this run from history? This cannot be undone.'
        }
        confirmLabel="Delete"
        loading={deleteBacktest.isPending}
