...
Live trading and paper operations moved to **`q_terminal`**, the native operations terminal (`./dev up terminal` or `cd q_terminal && make run`). Stale `/execution` links and the former `?deployment_id=` monitor window show a short notice in this research app.
