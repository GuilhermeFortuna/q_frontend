...
- **Backtests Workspace:** Configure and run strategies via the async backtest API, inspect OHLCV/trades charts with technical indicators, evaluate drawdown/equity curves, browse persisted run history, and export executive PDF reports.
...
- **Strategy Workspace:** Custom strategy workbench — blend a base entry strategy with composable exit rules and save named presets. Layout: compact saved-strategies rail (`lg:col-span-3`), wider form panel (`lg:col-span-9`), three-step flow (Setup → Entry → Exits), collapsible thesis, compact entry hints, and a sticky save/summary bar with Backtest staging. The exit section is a metadata-driven toggle-card configurator from `GET /api/v1/exit-rules`. Save payload is unchanged: `{ name, base_strategy, description, parameters }`.
